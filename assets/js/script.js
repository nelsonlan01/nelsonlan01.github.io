...
// collect the page names that correspond to nav links
const navPageNames = [...navigationLinks].map(link => link.innerHTML.toLowerCase());

for (let i = 0; i < navigationLinks.length; i++) {
  navigationLinks[i].addEventListener("click", function () {

    // Deactivate ALL sub-pages that are not direct nav targets
    // (covers project pages, material pages, and any other injected pages)
    document.querySelectorAll("article[data-page]").forEach(p => {
      if (!navPageNames.includes(p.dataset.page)) {
